...
type ModuleLoginFormProps = {
    title: string;
    description?: string;
    onVerify: (password: string) => Promise<{ success: boolean; message?: string }>;
    onSuccess: () => void;
}

export function ModuleLoginForm({ title, description, onVerify, onSuccess }: ModuleLoginFormProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
        ...
        toast({ title: 'Error', description: 'Error al verificar la contraseña.', variant: 'destructive' });
    } finally {
  ...
  return (
    <div className="flex items-center justify-center min-h-[60vh] bg-background/50 py-12">
      <Card className="w-full max-w-[450px] shadow-2xl border-none p-4 md:p-8">
        <CardHeader className="text-center space-y-4 mb-4">
          ...
          <CardDescription className="text-base text-muted-foreground">
            {description || 'Este módulo es de acceso restringido. Ingresa la contraseña.'}
          </CardDescription>
        </CardHeader>
