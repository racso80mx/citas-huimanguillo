...
import { verifyWarehousePassword } from '@/lib/actions';

type LoginFormProps = {
    onLoginSuccess: () => void;
}

export function WarehouseLoginForm({ onLoginSuccess }: LoginFormProps) {
  ...
  const handleLogin = async () => {
    setIsVerifying(true);
    const result = await verifyWarehousePassword(password);
    if (result.success) {
        onLoginSuccess();
      ...
      <Card className="w-full max-w-[450px] shadow-2xl border-none p-4 md:p-8">
        <CardHeader className="text-center space-y-4 mb-4">
          ...
          <CardDescription className="text-base text-muted-foreground">
            Este módulo es de acceso restringido para personal de Almacén.
          </CardDescription>
        </CardHeader>
