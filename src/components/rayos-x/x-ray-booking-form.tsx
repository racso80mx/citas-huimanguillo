...
import { saveNewXRayAppointment, getPatientByCURP, getModuleSettings } from '@/lib/actions';
    ...
    startTransition(async () => {
      ...
      if (result.success && result.data) {
        ...
        if (settings.rayosXWhatsAppEnabled) {
            ...
            const wsMessage = encodeURIComponent(`Hola ${data.name}, le contactamos del Hospital General de Huimanguillo para confirmar su cita de Rayos X con folio ${result.data.appointment.appointmentNumber} para el día ${formattedDateText} a las ${selectedTime} hrs. Estudio: ${selectedStudy.name}.${obs}`);
